...
PlusPCA <- function(inMat = distribution_test_mat(),
  ...
  #legendText = as.character(unique(out_df[,1]))

  #legendColors = sapply(unique(out_df[,2]), color.id)

  #legendColors = unique(out_df[,2])

  ColorVector = out_df[,1]

  in_data_imputed[,dim(in_data_imputed)[2]+1] <- ColorVector
